...
async function extractCharacterFeatures(base64, mediaType) {
  ...
  // Menggunakan model Gemini 1.5 Flash (sangat cepat dan bagus untuk gambar)
  const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });

  const systemInstruction = "You are a visual character analyst for AI image generation. Analyze the uploaded image and return ONLY a valid JSON object. No preamble, no markdown backticks, no explanation — raw JSON only.";
  
  const userPrompt = `Extract precise visual details from this person image for use in AI image generation prompts. Return ONLY valid JSON:
...
// Format gambar untuk Gemini API
  ...
  try {
    // Mengirim gambar dan prompt ke Gemini
    ...
    // Membersihkan teks jika AI menyelipkan markdown backticks (```json)
